...
exports.renderVarComponent = renderVarComponent;
exports.renderStepComponent = renderStepComponent;
exports.renderCheckComponent = renderCheckComponent;
exports.renderVarTableComponent = renderVarTableComponent;
exports.renderRefStepComponent = renderRefStepComponent;
exports.renderRefVarComponent = renderRefVarComponent;
exports.renderVariableComponent = renderVariableComponent;
/**
 * 将变量名转换为友好显示格式
 */
 ...
 */
function parseTypedParams(params) {
    const result = {};
    if (!params)
        return result;
...
function renderRichTooltip(displayName, escapedName, type, params) {
    const description = params.description || '';
    // 构建约束列表
    const constraints = [];
    if (params.ge !== undefined)
        constraints.push(`≥ ${params.ge}`);
    if (params.gt !== undefined)
        constraints.push(`> ${params.gt}`);
    if (params.le !== undefined)
        constraints.push(`≤ ${params.le}`);
    if (params.lt !== undefined)
        constraints.push(`< ${params.lt}`);
    if (params.max_length)
        constraints.push(`Max len: ${params.max_length}`);
    if (params.min_length)
        constraints.push(`Min len: ${params.min_length}`);
 ...
 */
function renderVarComponent(variable) {
 ...
 * 渲染 step 类型为时间轴节点
 */
function renderStepComponent(variable, stepIndex, isLast) {
 ...
 * 渲染 check 类型为行内复选框组件
 */
function renderCheckComponent(variable, description) {
    const displayName = formatVariableName(variable.name);
    const escapedName = variable.name.replace(/"/g, '&quot;');
    const escapedFullMatch = variable.fullMatch.replace(/"/g, '&quot;');
    const params = parseTypedParams(variable.params || '');
 ...
 */
function renderVarTableComponent(variable) {
    const displayName = formatVariableName(variable.name);
    const escapedName = variable.name.replace(/"/g, '&quot;');
    const escapedFullMatch = variable.fullMatch.replace(/"/g, '&quot;');
    const params = parseTypedParams(variable.params || '');
    ...
    if (params.subvars) {
        ...
        let bLevel = 0;
        let inQ = null;
        const subvarsStr = params.subvars;
 ...
 */
function renderRefStepComponent(variable) {
    const params = parseTypedParams(variable.params || '');
    const displayName = params.title || formatVariableName(variable.name);
    const escapedName = variable.name.replace(/"/g, '&quot;');
 ...
 */
function renderRefVarComponent(variable) {
    const params = parseTypedParams(variable.params || '');
    const displayName = params.title || formatVariableName(variable.name);
    const escapedName = variable.name.replace(/"/g, '&quot;');
 ...
 * 根据变量类型渲染对应的组件
 */
function renderVariableComponent(variable, context) {
    switch (variable.type) {
        case 'var':
            return renderVarComponent(variable);
        case 'step':
            return renderStepComponent(variable, context?.stepIndex || 0, context?.isLast || false);
        case 'check':
